...
function Poly1305KeyGen(key::Vector{UInt8}, nonce::Vector{UInt8})
    return unsafe_wrap(Array{UInt8}, pointer(Chacha20.XChacha20Block(Chacha20.newXChachaState(key, 0x0000000000000000, nonce))), 32)
end

function Encrypt(ciphertext::Vector{UInt8}, key::Vector{UInt8}, nonce::Vector{UInt8}, text::Vector{UInt8}, add::Vector{UInt8})
    nbytes = UInt64(Chacha20.XChacha20Encrypt(ciphertext, key, 0x0000000000000001, nonce, text))
...
function Decrypt(text::Vector{UInt8}, key::Vector{UInt8}, nonce::Vector{UInt8}, ciphertext::Vector{UInt8}, add::Vector{UInt8})
    ...
    nbytes = Chacha20.XChacha20Decrypt(text, key, 0x0000000000000001, nonce, unsafe_wrap(Array{UInt8}, pointer(ciphertext), len))
